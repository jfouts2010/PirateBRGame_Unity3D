...
public class Crosshair : MonoBehaviour
{
    ...
    private vp_FPWeaponShooter WeaponShooter;
    private vp_Weapon CurrentWeapon;
    void Start()
    {
        ...
        verticalTexture = (Texture2D)gameObject.GetComponent<vp_SimpleCrosshair>().m_dynamicCrosshair;
        horizontalTexture = (Texture2D)gameObject.GetComponent<vp_SimpleCrosshair>().m_dynamicCrosshair;
        
    }

    void Update()
    {
        CurrentWeapon = gameObject.GetComponent<vp_FPWeaponHandler>().CurrentWeapon;
        if(CurrentWeapon != null)
        {
            WeaponShooter = CurrentWeapon.gameObject.GetComponent<vp_FPWeaponShooter>();
            if (WeaponShooter != null)
                spread = WeaponShooter.ProjectileTotalSpread*20;
        }
    }

    void OnGUI()
    {
        if (m_Player.Zoom.Active && m_Player.IsFirstPerson.Get())
            return;
