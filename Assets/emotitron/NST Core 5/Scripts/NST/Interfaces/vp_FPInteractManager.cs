...
public class vp_FPInteractManager : MonoBehaviour
{
    ...
    public vp_Interactable m_CurrentInteractable = null; // for caching what the player is currently interacting with
    ...
    public virtual void LateUpdate()
    {
        ...
        {
            if (m_Player.InputGetButton.Send("Interact"))
            {
                m_CurrentInteractable.TryInteract(m_Player);
            }
        }
    }
    public void Update()
    ...
    protected virtual bool CanStart_Interact()
    {
        ...
        if (m_LastInteractable != null)
        {
            ...
            m_Player.Velocity.Set(new Vector3(0, 0, 0));
            m_Player.SetState("Default", true);
            m_Player.SetState("Run", false);
            // reset some things
    ...
    protected virtual void InteractCrosshair()
    {
        ...
        if (FindInteractable(out interactable))
        {
            ...
            //always update
            if (interactable.InteractTextObject != null)
            {
                //display it!
                interactable.InteractTextObject.SetActive(true);
                //set the gameobject to look at you
                interactable.InteractTextObject.transform.LookAt(m_Camera.transform);
